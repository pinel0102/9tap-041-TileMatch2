...
namespace I2.SmartEdge
{
    ...
    public class SE_AnimSequence_Position : SE_AnimSequence
    {
        ...
        public override void Apply_Characters(SmartEdge se, SE_Animation anim, int sequenceIndex)
        {
            ...
            {
                from.x = Mathf.LerpUnclamped(se.mAllCharactersMin.x, se.mAllCharactersMax.x, _From.x);
                from.y = Mathf.LerpUnclamped(se.mAllCharactersMin.y, se.mAllCharactersMax.y, _From.y);
            }

            if (_AnimBlend_To == eAnimBlendMode.Replace)
            {
                to.x = Mathf.LerpUnclamped(se.mAllCharactersMin.x, se.mAllCharactersMax.x, _To.x);
                to.y = Mathf.LerpUnclamped(se.mAllCharactersMin.y, se.mAllCharactersMax.y, _To.y);
            }
            ...
            for (int iElement = mElementRangeStart; iElement < mElementRangeEnd; ++iElement)
            {
                ...
                var currentValue = SmartEdge.mOriginalVertices.Buffer[iElement*4].position;
                ...
                for (int v = iElement * 4; v < iElement * 4 + 4; ++v)
                {
                    ...
                    if (_ApplyX) SmartEdge.mOriginalVertices.Buffer[v].position.x = newValue.x    + (SmartEdge.mOriginalVertices.Buffer[v].position.x- currentValue.x);
                    if (_ApplyY) SmartEdge.mOriginalVertices.Buffer[v].position.y = newValue.y    + (SmartEdge.mOriginalVertices.Buffer[v].position.y - currentValue.y);
                    if (_ApplyZ) SmartEdge.mOriginalVertices.Buffer[v].position.z = newValue.z    + (SmartEdge.mOriginalVertices.Buffer[v].position.z - currentValue.z);
                }
            }
        }
