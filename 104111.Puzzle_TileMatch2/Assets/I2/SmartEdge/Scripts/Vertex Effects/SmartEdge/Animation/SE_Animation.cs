...
namespace I2.SmartEdge
{
    public class SE_Animation
    {
        ...
        [NonSerialized][XmlIgnore] public int mNumElements; // Num elements in the SmartEdge component (used to know when the InitTime should be executed again if the text changes)
        ...
        public void Play(SmartEdge se)
        {
            if (se != null)
            {
                ...
                SmartEdgeManager.RegisterAnimation(se);
        ...
        public void Stop( SmartEdge se, bool executeCallback = true )
        {
            if (IsPlaying)
            {
        ...
        public void AnimUpdate(float dt, SmartEdge se, ref bool makeMaterialDirty, ref bool makeVerticesDirty)
        {
            if (se == null)
                return;
        ...
        public void Apply_Characters(SmartEdge se)
        {
            InitTimes(se);
            if (mNumElements <= 0)
                return;
        ...
        public void Apply_Vertices(SmartEdge se)
        {
            InitTimes(se);
            if (mNumElements <= 0)
                return;
            ...
            for (int i = 0, imax = _Sequences.Length; i < imax; ++i)
                if (_Sequences[i]._Enabled)
                    _Sequences[i].Apply_Vertices(se, this, i);
        }


        public void InitTimes(SmartEdge se, bool force=false)
        {
            if (mRandomSeed < 0)
                mRandomSeed = DRandom.GetSeed();

            int nElements = (se == null) ? 15 : SmartEdge.mCharacters.Size;
        ...
        static System.Type[] mAnimSequenceTypes = {
            typeof(SE_AnimSequence_Alpha), typeof(SE_AnimSequence_AlphaFace), typeof(SE_AnimSequence_AlphaOutline), typeof(SE_AnimSequence_AlphaShadow), typeof(SE_AnimSequence_AlphaGlow),
