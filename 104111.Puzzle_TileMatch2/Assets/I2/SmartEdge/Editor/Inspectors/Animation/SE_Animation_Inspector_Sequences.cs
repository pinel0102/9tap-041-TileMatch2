...
/*                            if (mCurrentSequenceIndex>=0 && !mSequence_IsDragging && currentEvent.button == 0 && rect.Contains(currentEvent.mousePosition) )
        ...
        void OnGUI_AnimationControls( Rect rect, float SegmentsMinY )
        {
            ...
            if (EditorGUI.EndChangeCheck() && mSmartEdge != null)
            {
                mSmartEdge.MarkWidgetAsChanged(dirtyVertices, dirtyMaterials);
            }
        ...
        {
            if (mTimeLine_PlayingStartTime <= 0 || mAnimation == null || mSmartEdge == null)
            {
                UnRegisterUpdatePlay();
                mTimeLine_PlayingStartTime = -1;
                return;
            ...
            bool dirtyMaterials = false;
            mAnimation.IsPlaying = true;
            mAnimation.AnimUpdate(Time.realtimeSinceStartup - mTimeLine_PlayingStartTime, mSmartEdge, ref dirtyVertices, ref dirtyMaterials);

            if (!mAnimation.IsPlaying)
            {
                //AnimationControls_StopPlaying(true);
                //mAnimation.mRealTime = mAnimation.mTime = 0;
                mAnimation.Play(mSmartEdge);
                mTimeLine_PlayingStartTime = -1;
            }
            else
                mTimeLine_PlayingStartTime = Time.realtimeSinceStartup;
        ...
        void OnGUI_Animation_SequenceElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (isPaintEvent)
            {
                ...
                GUI.Label(rectName, seq._Name, SE_InspectorTools.Style_LabelItalic);
            }
        ...
        void OnGUI_OnAddSequence()
        {
            ...
            menu.AddSeparator("");
            menu.AddItem(new GUIContent("Basic/Alpha/Global"), false, AddBasicSequence, typeof(SE_AnimSequence_Alpha));
            menu.AddItem(new GUIContent("Basic/Alpha/Face"), false, AddBasicSequence, typeof(SE_AnimSequence_AlphaFace));
            menu.AddItem(new GUIContent("Basic/Alpha/Outline"), false, AddBasicSequence, typeof(SE_AnimSequence_AlphaOutline));
            menu.AddItem(new GUIContent("Basic/Alpha/Glow"), false, AddBasicSequence, typeof(SE_AnimSequence_AlphaGlow));
            menu.AddItem(new GUIContent("Basic/Alpha/Shadow"), false, AddBasicSequence, typeof(SE_AnimSequence_AlphaShadow));
            ...
            var mAnimPath = SmartEdge_Inspector.GetI2SmartEdgePath() + "/Presets/Animations/";
        ...
        void DuplicateSequence(object o)
        {
            ...
            mAnimation.InitTimes(mSmartEdge, true);
            SE_AnimationPreset_Inspector.mDirty = true;
        }
        ...
        void AddPresetSequence(object o)
        {
            ...
            var list = mAnimation._Sequences.ToList();
            list.InsertRange(idx, sequences);
            mAnimation._Sequences = list.ToArray();
