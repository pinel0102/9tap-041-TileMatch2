...
#else
    public partial class SmartEdge : IVertexModifier, IMaterialModifier
	{
        ...
        private void ExportVerticesToUGUI(ref List<UIVertex> verts)
        {
            ...
            for (int layer = 0; layer < mSEMesh.numLayers; ++layer)
            {
                var mesh = mSEMesh.mLayers[layer];
                for (int i = 0; i < mesh.Size; ++i)
                {
                    uiVertex.position = mesh.Buffer[i].position;
                    uiVertex.color    = mesh.Buffer[i].color;
                    uiVertex.uv0      = mesh.Buffer[i].uv;
                    uiVertex.uv1      = mesh.Buffer[i].uv1;
                    uiVertex.normal   = mesh.Buffer[i].normal;
                    uiVertex.tangent  = mesh.Buffer[i].tangent;
                    verts.Add(uiVertex);
                }
            }
        }
