...
public class BoardView : MonoBehaviour
{
	...
	public void OnUpdateBrushWidget(BrushMode mode, Vector2 localPosition, bool interactable)
	{
		...
		m_brush.UpdateUI(interactable, mode);
	}
    ...
    public void OnInvisibleLayer(IReadOnlyList<int> invisibleList)
    {
		for (int i = 0, count = m_placedLayerObjects.Count; i < count; i++)
		{
			var layerView = m_placedLayerObjects[i];
			layerView.OnVisible(!invisibleList.Contains(i));
		}
    }
}
